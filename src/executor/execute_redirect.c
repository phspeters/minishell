...
/*   Updated: 2024/08/28 17:01:50 by pehenri2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	execute_redirect(t_tree_node *left, t_tree_node *right, int redir_type)
{
	...
	fd = -1;
	before_expansion = right->tokens->value;
	expand_tokens(right);
	if ((*before_expansion != '\0' && !right->tokens) || right->tokens->next)
...
int	open_redir_file(t_tree_node *right, int redir_type, int *fd)
{
	if (redir_type == REDIR_APPEND)
		*fd = open(right->tokens->value, O_CREAT | O_APPEND | O_WRONLY, 0644);
	else if (redir_type == REDIR_HEREDOC || redir_type == REDIR_IN)
		*fd = open(right->tokens->value, O_RDONLY, 0644);
	else if (redir_type == REDIR_OUT)
		*fd = open(right->tokens->value, O_CREAT | O_TRUNC | O_WRONLY, 0644);
	if (*fd == -1)
		return (handle_error(right->tokens->value));
	return (SUCCESS);
}
