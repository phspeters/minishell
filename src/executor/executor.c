...
/*   Updated: 2024/08/28 17:03:30 by pehenri2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	executor(t_tree_node *root)
{
	if (!root->tokens)
		return (SUCCESS);
	if (root->tokens->type == AND)
		return (execute_and(root->left, root->right));
	else if (root->tokens->type == OR)
		return (execute_or(root->left, root->right));
	else if (root->tokens->type == PIPE)
		return (execute_pipe(root->left, root->right));
	else if (root->tokens->type >= REDIR_APPEND
		&& root->tokens->type <= REDIR_OUT)
		return (execute_redirect(root->left, root->right, root->tokens->type));
	else if (root->tokens->type == OPEN_PAREN)
...
int	execute_block(t_tree_node *root)
{
	...
	if (pid == 0)
	{
		...
		if (parser(root->tokens, &block_root) == SUCCESS)
